...
<div class="flex flex-col h-full">
  ...
  <div class="flex-1 overflow-y-auto">
    {#if sessions.length === 0}
      <p class="text-center text-gray-500 text-sm p-4">Keine Chats vorhanden</p>
    {:else}
      <ul class="py-2">
        {#each sessions as session (session.id)}
          <li
            class="group px-3 py-2 hover:bg-gray-800 transition-colors cursor-pointer
            ...
            {#if renamingId === session.id}
              <div class="flex gap-1">
                <input
                  ...
                  class="flex-1 text-sm bg-gray-700 border border-gray-600 rounded px-2 py-0.5
            ...
            {:else}
              ...
              >
                <div class="flex-1 min-w-0">
                  <p class="text-sm text-gray-200 truncate">{session.title}</p>
                  <p class="text-xs text-gray-500">{formatDate(session.updated_at)}</p>
                </div>
