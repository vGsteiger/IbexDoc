...
fn run_migrations(conn: &Connection) -> Result<(), AppError> {
    ...
    // Migration 10: Model registry for multi-model management
    if version < 10 {
        log::info!("Running migration 010: Model registry");
        conn.execute_batch(include_str!("migrations/010_model_registry.sql"))?;
